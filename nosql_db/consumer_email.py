...
QUEQUE_NAME = RemaindBy.email.name
...
def main():
    def proccess_message(ch, method, properties, body):
        ...
        if send_email(contact.email, message):
            contact.sent = True
            contact.save()
            print(f"{GREEN} [X] Successfully sent to {contact.email}")
        else:
    ...
    with get_channel() as channel:
        ...
        channel.basic_consume(queue=QUEQUE_NAME, on_message_callback=proccess_message)
